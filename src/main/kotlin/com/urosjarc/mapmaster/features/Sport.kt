...
data class SportFeatures(
    val nodes: MutableList<SportNode> = mutableListOf(),
    val ways: MutableList<SportWay> = mutableListOf(),
    val rels: MutableList<SportRel> = mutableListOf()
) {
    fun add(feature: OsmFeature) {
        val enumValue = feature.obj.tags["sport"]
        val type = SportType.entries.firstOrNull { it.value == enumValue }
            ?: SportType.OTHER
        when (feature.objType) {

            OsmFeature.Type.NODE ->
                this.nodes.add(SportNode(node = feature.obj as OsmNode, type = type))

            OsmFeature.Type.RELATIONSHIP ->
                this.rels.add(SportRel(rel = feature.obj as OsmRel, type = type))

            OsmFeature.Type.WAY ->
                this.ways.add(SportWay(way = feature.obj as OsmWay, type = type))
