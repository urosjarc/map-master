...
data class BarrierWay(
    val way: OsmWay,
    val type: BarrierType
)

data class BarrierRel(
    val rel: OsmRel,
    val type: BarrierType
)

data class BarrierFeatures(
    val nodes: MutableList<BarrierNode> = mutableListOf(),
    val ways: MutableList<BarrierWay> = mutableListOf(),
    val rels: MutableList<BarrierRel> = mutableListOf()
) {
    fun add(feature: OsmFeature) {
        val enumValue = feature.obj.tags["barrier"]
        val type = BarrierType.entries.firstOrNull { it.value == enumValue }
            ?: BarrierType.OTHER
        when (feature.objType) {

            OsmFeature.Type.NODE ->
                this.nodes.add(BarrierNode(node = feature.obj as OsmNode, type = type))

            OsmFeature.Type.RELATIONSHIP ->
                this.rels.add(BarrierRel(rel = feature.obj as OsmRel, type = type))

            OsmFeature.Type.WAY ->
                this.ways.add(BarrierWay(way = feature.obj as OsmWay, type = type))

        }
    }
}

enum class BarrierType(val value: String) {
