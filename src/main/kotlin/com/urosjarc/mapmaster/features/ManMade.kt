...
data class ManMadeNode(
    val node: OsmNode,
    val type: ManMadeType
)

data class ManMadeWay(
    val way: OsmWay,
    val type: ManMadeType
)

data class ManMadeRel(
    val rel: OsmRel,
    val type: ManMadeType
)

data class ManMadeFeatures(
    val nodes: MutableList<ManMadeNode> = mutableListOf(),
    val ways: MutableList<ManMadeWay> = mutableListOf(),
    val rels: MutableList<ManMadeRel> = mutableListOf()
) {
    fun add(feature: OsmFeature) {
        val enumValue = feature.obj.tags["man_made"]
        val type = ManMadeType.entries.firstOrNull { it.value == enumValue }
            ?: ManMadeType.OTHER
        when (feature.objType) {

            OsmFeature.Type.NODE ->
                this.nodes.add(ManMadeNode(node = feature.obj as OsmNode, type = type))

            OsmFeature.Type.RELATIONSHIP ->
                this.rels.add(ManMadeRel(rel = feature.obj as OsmRel, type = type))

            OsmFeature.Type.WAY ->
                this.ways.add(ManMadeWay(way = feature.obj as OsmWay, type = type))

        }
    }
}

enum class ManMadeType(val value: String) {
